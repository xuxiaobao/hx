...
import com.miaosu.service.recharge.sup.BeijingzhiyunRecharge;
 ...
 */
@Controller
@RequestMapping("/notify/beijingzhiyun")
public class BeijingzhiyunNotifyController
{
	...
	@Autowired
	private BeijingzhiyunRecharge beijingzhiyunRecharge;
	...
	public String orderStatus(HttpServletRequest request, HttpServletResponse response,String xmlhead,String xmlbody) throws IOException
	{
		...
		notifyLog.info("beijingzhiyun充值结果通知xmlhead{}", xmlhead);
		notifyLog.info("beijingzhiyun充值结果通知xmlbody{}", xmlbody);
		Order order =null;
		try{
			...
			notifyLog.info("beijingzhiyun充值结果通知OperSeq{}", OperSeq);
			notifyLog.info("beijingzhiyun充值结果通知FailNum{}", FailNum);
			...
			beijingzhiyunRecharge.callBack(rechargeResult);
		...
		sb.append("<TransIDO>"+"BJZY"+transIDOTime+"000000000000"+"</TransIDO>");
		sb.append("<TransIDOTime>"+transIDOTime+"</TransIDOTime>");
		sb.append("<TransIDH>"+"BJZY"+transIDOTime+"000000000000"+"</TransIDH>");
...
//		notifyLog.info("beijingzhiyun充值结果通知：{}", IOUtils.toString(request.getInputStream()));
//		notifyLog.info("beijingzhiyun充值结果通知：{}", request.getParameter("message"));
