 ...
 */
@Service
public class HongxinTestRecharge extends AbstractRecharge
{
...
//	@Value("${zhuowang.server}")
	private String SERVER_URL = "http://123.56.203.67:81/HXTestRecharge/upperReachesServlet";
...
//		map.put("key", "0da7e2c41773078e0ee37ade52889885");
		...
		try
		{
			...
			if (entity != null)
			{
				...
				logger.info("hongxinTest 充值同步返回结果{}", resultString);
		...
		if (result.getString("state") != null)
		{
			if ("success".equals(result.getString("state")))
			{
				...
				abstractOrderService.setRechargeId(order.getId(), "", "hongxinTest");
			}
			else
			{
