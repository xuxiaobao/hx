 ...
 */
@Service
public class BeijingzhiyunRecharge extends AbstractRecharge
{
...
//	@Value("${zhuowang.server}")
	...
	private String APP_KEY="ZHIYUNKYKJ@NM";
	private String APP_SECRET="ZaAu1UbOehvcwkDnP4dLf9pn";
...
//		JSONObject result = null;
    	...
    	HttpPost httppost=new HttpPost("http://111.13.21.128:80/ec_serv_intf/forec");
    	
    	String bipCode = "BIP4B876";
    	String activityCode = "T4011137";
    	
    	String PRODUCT_ID = "9003674663";
    	
    	String TransIDO = "BJZY";
...
//    	String msgHeaer = createReqHeader(bipCode, activityCode);
    	...
    	sb.append("<TransIDO>");
    	sb.append(TransIDO+transIDOTime+"000000000000");
    	sb.append("</TransIDO>");
    	...
    	logger.info("beijingzhiyun 充值同步xmlhead{}", xmlhead);
    	...
    	logger.info("beijingzhiyun 充值同步xmlbody{}", xmlbody);
    	sb.insert(0, '\n');
    	sb.insert(0, xmlhead);
     ...
     //   RequestConfig requestConfig = RequestConfig.custom()
        ...
        try {
            ...
            if (resEntity != null) {
                ...
                logger.info("beijingzhiyun 充值同步响应报文{}", bossRspStr);
...
//                eCXmlRespose = (ECXmlRespose) xStream.fromXML(bossRspStr);
	...
	//	            System.out.println("OperSeq:" + OperSeq);
				...
				if ("00".equals(Status))
				{
					...
					if (StringUtils.isEmpty(rechargId))
					{
						logger.warn("beijingzhiyun充值返回rechargeId为空");
						throw new ServiceException(ResultCode.FAILED);
					}
					abstractOrderService.setRechargeId(order.getId(), rechargId, "beijingzhiyun");
				}
				else
				{
					String failedReason = "";
...
//		} 
	...
	public void setAuthHeaders(HttpPost httppost, String appKey, String appSecret, String product, String activicode)
	{
		...
		logger.info("beijingzhiyun 充值同步created{}", created);
		logger.info("beijingzhiyun 充值同步pd{}", pd);
		logger.info("beijingzhiyun 充值同步wsse{}", wsse);
