...
Ext.ux.grid.ColumnHeaderGroup = Ext.extend(Ext.util.Observable, {
        ...
        renderHeaders: function(){
            ...
            for(var row = 0, rlen = rows.length; row < rlen; row++){
                ...
                for(var i = 0, gcol = 0, len = r.length; i < len; i++){
                    ...
                    //var width = gs.width.substring(0, gs.width.lastIndexOf("px")) - 2;
    ...
    getGroupStyle: function(group, gcol){
        ...
        //console.log("**1:" + width);
        return {
            width: (Ext.isBorderBox || (Ext.isWebKit && !Ext.isSafari2) ? width - 2 : Math.max(width - this.borderWidth - 2, 0)) + 'px',
            hidden: hidden
        };
    },
