 ...
 * Created by angus
 */
DiscountPanel = function (config) {

    var serviceUrl = Desktop.contextPath + "/api/userDiscount/search";
    var me = this;
    ...
    gridColumns.push({id: 'pid', align: 'center', header: "运营商", width: 100, sortable: true, dataIndex: 'pid'});
    gridColumns.push({id: 'discount', align: 'center', header: "折扣", width: 140, sortable: true, dataIndex: 'discount'});
    ...
    function addBtn(){
        var win = Ext.getCmp("win.addDiscount");
        var me = this;
        if (!win) {
            win = new AddDiscountWindow({
                id: 'AddDiscountWin',
                modal: true,
                callback: function(){
    ...
    function updateBtn(){
        ...
        var win = Ext.getCmp("editDiscountWindow");
        if (!win) {
            win = new EditDiscountWindow({
                id: 'editDiscountWindow',
                modal: true,
                data: s[0],
                callback: function(){
                    this.gridPanel.store.load();
    ...
    DiscountPanel.superclass.constructor.call(this, allConfig);
};

Ext.extend(DiscountPanel, Ext.Panel, {
    /**
     * 快速选择时间按钮动作
...
Ext.reg('tab.discount', DiscountPanel);
